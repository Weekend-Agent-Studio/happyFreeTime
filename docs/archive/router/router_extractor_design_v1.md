...
_归档状态：RouterExtractor 早期版本 · 仅保留设计演进_

---

> ⚠️ **历史文档：** 不得据此自动修改当前 Router。当前职责和数据契约以 V2 架构、当前代码和测试为准。
...
User Input
  -> RouterExtractor
       一次 LLM 调用：识别意图、抽取显式约束、识别用户选择
  -> Enrichment Service
       代码补全：当前时间、默认位置、天气、默认预算、默认距离、默认时间窗
  -> NeedQuestion Gate
       代码判断：缺关键字段才 interrupt 反问
...
Planner 和 Executor 暂时可以沿用现有实现，后续再逐步改造成 service-first 的规划方式。
...
```text
RouterExtractor: 1 次 LLM
Enrichment Service: 0 次 LLM
NeedQuestion Gate: 0 次 LLM
```

理想情况下，一个普通规划请求在进入 Planner 前只需要一次 LLM。
...
need_question_gate:
  need_question = true -> interrupt -> router_extractor 或 enrichment
...
User
  -> RouterExtractor
       output: intent + raw_constraints
  -> EnrichmentService
       output: constraints + assumptions
  -> NeedQuestionGate
       if blocking missing:
         interrupt(question)
         merge user answer into user_input/context
         rerun RouterExtractor or rerun Enrichment
       else:
         Planner
```
...
1. 新增 RouterExtractor，但不接入 Graph，先用测试脚本验证输出。
2. 新增 Enrichment 和 Gate，验证它们能产出现有 Planner 可用的 `slot/constraints`。
3. 修改 `Agents/graph.py`，把 `intent -> slot` 替换为 `router -> enrichment -> gate`。
